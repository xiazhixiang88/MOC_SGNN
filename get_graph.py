...
def Cancer_Survival_train_val(config):
    all_paths = glob.glob(config['out_dir']+'/homogeneous/*.pkl')
    graphs = []
    for p in all_paths:
        graphs.append(p)
...
def main():
    ...
    if CONSTRUCT:
        ...
        for i, wsi_input in enumerate(patch_paths):
            ...
            graph_constructor = GraphConstructor(graph_config, hovernet_config, kimianet_config, uni_config,
                                                 wsi_input, tail, GET_FEATURES)
            # het_graph, homo_graph, node_type = graph_constructor.construct_graph()
            homo_graph = graph_constructor.construct_graph()
    ...
    if GET_TRAINVAL:
        ...
        list_name_survival = f"/list_survival_f{fold}/"
        if graph_config['dataset'] == 'COAD':
            ...
            if graph_config['task'] == "cancer classification":
                ...
                raise ValueError("No such task")
        elif graph_config['dataset'] == 'SUR':
            train_list, val_list, test_list = Cancer_Survival_train_val(graph_config)
            list_name = list_name_survival
