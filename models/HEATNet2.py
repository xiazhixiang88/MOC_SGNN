...
class HEATNetLayer(nn.Module):
    def __init__(self, in_size, out_size, n_heads=1, dropout=0.2):
        super(HEATNetLayer, self).__init__()
        self.gcn = GCNConv(in_size, out_size)
...
class HEATNet2(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, n_layers, dropout=0.2, graph_pooling_type='mean'):
        super(HEATNet2, self).__init__()
        self.layers = nn.ModuleList()
        self.layers.append(HEATNetLayer(in_dim, hidden_dim, dropout=dropout))

        for _ in range(n_layers - 1):
            self.layers.append(HEATNetLayer(hidden_dim, hidden_dim, dropout=dropout))

        self.out_layer = nn.Linear(hidden_dim, out_dim)

        # Define pooling layer
        if graph_pooling_type == 'mean':
            self.pooling = global_mean_pool
        else:
            raise NotImplementedError("Currently only supports mean pooling")

    def forward(self, data):
        # Assume data is a torch_geometric Data object
