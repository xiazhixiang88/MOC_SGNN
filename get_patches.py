...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', type=str, default='WSI_svs', help='Dataset name')
    ...
    parser.add_argument('-m', '--magnifications', type=int, nargs='+', default=(2,),
    ...
    if len(levels) == 2:
        ...
        out_base = os.path.join('data', 'patches', args.dataset, f'patches_lv{levels[0]}_ps256')
    ...
    import pandas as pd
    remove_already = True
    slides_df = pd.read_excel('data/survival list.xlsx')
    slides_name = slides_df['病理号'].tolist()
    
    processed_path = f"./data/patches/WSI_svs/patches_lv{levels[0]}_ps256/SUR"

    # 过滤出未处理且在 slides_name 中的 slides
    # 检查已经处理的 slides 并删除
    filtered_slides = []
    for slide in all_slides:
        ...
        # 检查是否存在相应文件夹
        processed_folder_path = os.path.join(processed_path, file_name_without_ext)
        if not os.path.exists(processed_folder_path):
            filtered_slides.append(slide)
        elif remove_already:
            print(f"Deleting {slide} (already processed)")
            os.remove(slide)

    # pos-i_pos-j -> x, y
    for idx, c_slide in enumerate(filtered_slides):
        print('Process slide {}/{}'.format(idx + 1, len(filtered_slides)))
    ...
    print('Patch extraction done for {} slides.'.format(len(filtered_slides)))